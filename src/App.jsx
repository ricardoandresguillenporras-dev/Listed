...
export default function SuperLista() {
  ...
  // Strategy (single-guard, WebView-safe):
  //   • On mount, push exactly ONE sentinel so every popstate is ours.
  //   • On every popstate (except intentional exit) immediately re-push the
  //     sentinel BEFORE doing any React state work — this keeps the guard
  //     alive across every press without stacking extra entries.
  //   • Use a ref for all nav state so the single listener is never stale.
  //   • Exit flow: on double-press at root, set the intentional-exit flag,
  //     call go(-1) to remove the sentinel, and let the WebView close when
  //     its own history is exhausted. We do NOT call go(-2) because that can
  //     skip past WebView-owned entries and behave unpredictably.
  ...
  useEffect(() => {
    // Single sentinel — all we ever need
    window.history.pushState({ sl: true }, "");

    const handlePopState = (e) => {
      // Intentional exit path: don't re-push, let WebView close
      if (intentionalExitRef.current) return;

      // Re-push sentinel IMMEDIATELY so the next back press is also caught.
      // Doing this before any setState avoids a race where a fast second press
      ...
      if (exitToastTimer.current) {
        ...
        // Remove only the sentinel we just re-pushed above, so the WebView
        // sees its own history exhausted and closes the activity naturally.
        window.history.go(-1);
        return;
      }

      // First press at root → show "press again to exit" toast
