...
import bgMoon  from "./assets/bg-purple.jpg";
                ...
                {addingAmt ? (
                  <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                    <input autoFocus type="number" value={newAmt} onChange={e => setNewAmt(e.target.value)}
                      ...
                      style={{ width:90, background:"#FFFFFF", border:"1.5px solid var(--accent)", borderRadius:20, padding:"7px 10px", color:"#2C2318", fontSize:13, outline:"none", textAlign:"center" }} />
...
function SwipeItem({ item, onToggle, onQtyMinus, onQtyPlus, onDelete, onContextMenu, editingPriceId, tempPrice, setTempPrice, setEditingPriceId, savePrice, sym }) {
  ...
  return (
    <div ref={wrapRef} className="item-stagger" style={{ position:"relative", overflow:"hidden", borderBottom:"1px solid rgba(0,0,0,0.04)", animation:"slItemSpring .30s var(--ease-spring) both" }}>
      ...
      <div ref={rowRef}
        style={{ display:"flex", alignItems:"center", padding:"11px 14px", gap:10, background:"#FFFFFF", position:"relative", touchAction:"pan-y", userSelect:"none", transition:"background .15s", cursor:"pointer" }}
        onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd}
        onMouseDown={onStart} onMouseMove={onMove} onMouseUp={onEnd} onMouseLeave={onEnd}
  ...
  return (
    <>
      ...
      <div style={Sth.body}>
        ...
        {unchecked.length > 0 && (
          <div style={{ display:"flex", alignItems:"center", padding:"8px 16px 6px", borderBottom: theme.isDark ? "1px solid #1E293B" : "1px solid #F0F2EF" }}>
            <button onClick={() => setShowUnchecked(v => !v)}
              ...
              <span>En lista</span>
              <span style={{ background: theme.isDark ? "#1A2550" : "#EDE8DF", borderRadius:10, padding:"1px 8px", fontSize:11, color: theme.isDark ? "#94A3B8" : "#8A8075", fontWeight:600 }}>{unchecked.length}</span>
            </button>
          </div>
        )}
        ...
        {checked.length>0 && (
          <div style={{ display:"flex", alignItems:"center", padding:"9px 12px 9px 16px", borderTop:"1px solid rgba(220,252,231,0.7)", background:"#EEF8EE", gap:8 }}>
      ...
      <div style={Sth.bottomBar}>
        <div style={{ position:"relative", flex:1 }}>
          ...
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar en lista..."
            style={{ width:"100%", background: theme.isDark ? "#1A2550" : "#FFFFFF", border: theme.isDark ? `2px solid #2E4484` : `2px solid ${theme.inputBorder||theme.border||"#C8DC90"}`, borderRadius:22, padding:"9px 16px 9px 36px", color: theme.isDark ? "#E2E8F0" : "#2C2318", fontSize:14, outline:"none", boxSizing:"border-box" }} />
        </div>
      </div>
...
function AddItemsView({ list, onBack, onAddItem, sym, theme = {} }) {
  ...
  return (
    <>
      ...
      {/* ── Custom item row ── */}
      <div style={{ padding:"14px 16px 12px", borderBottom:"1px solid #EBE8E2", background:"#FFFFFF" }}>
        ...
        {showEmojiPicker && (
          <div style={{ marginTop:12, background:"#FFFFFF", border:"1.5px solid #E0D8CE", borderRadius:16, padding:"12px 10px", animation:"fadeIn .15s ease" }}>
          ...
          filtered.map(p => {
            ...
            return (
              <button key={p.name} onClick={(e) => addPreset(p,e)}
                ...
                style={{ width:"100%", display:"flex", alignItems:"center", gap:14, padding:"12px 16px", background: theme.isDark ? "#1A2550" : "#FFFFFF", border:"none", borderBottom:`1px solid ${theme.isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)"}`, cursor:"pointer", color: theme.isDark ? "#E2E8F0" : "#1A2118", textAlign:"left", transition:"background .12s ease" }}
                onMouseEnter={e => e.currentTarget.style.background=theme.isDark ? "#1E2D60" : theme.soft}
                onMouseLeave={e => e.currentTarget.style.background=theme.isDark ? "#1A2550" : "#FFFFFF"}
                onTouchStart={e => e.currentTarget.style.background=theme.isDark ? "#1E2D60" : theme.soft}
                onTouchEnd={e   => e.currentTarget.style.background=theme.isDark ? "#1A2550" : "#FFFFFF"}>
...
/* heartbeat breathe — use animation-play-state to pause on hover/active */
.wc-fab { animation: slFabBreath 3s ease-in-out infinite; }
