...
function ListView({ list, onBack, onUpdateItem, onDeleteItem, onGoAdd, sym, budget, onOpenProfile, onSaveBudget, onCloseSession, theme = {}, searchQuery = "", onSearchChange }) {
  ...
  const { all, unchecked, checked, done, tot, inBagCost, remaining, overBudget, budgetPct } = useMemo(() => {
    ...
    const all       = searchQuery ? list.items.filter(i => i.name.toLowerCase().includes(sq)) : list.items;
...
function AddItemsView({ list, onBack, onAddItem, sym, theme = {} }) {
  ...
  const filtered = useMemo(() => {
    ...
    return PRESET_ITEMS.filter(p => {
      ...
      const inSearch = p.name.toLowerCase().includes(sq);
    ...
    <div style={{ display:"flex", flexDirection:"column", flex:1, minHeight:0 }}>
      ...
      <div style={{ padding:"14px 16px 12px", borderBottom:`1px solid ${theme.isDark ? theme.border : "#EBE8E2"}`, background: theme.isDark ? theme.surface : theme.cardBg }}>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <button onClick={() => setShowEmojiPicker(v => !v)}
            style={{ width:52, height:52, background: showEmojiPicker ? "var(--accent)" : theme.isDark ? theme.soft : "#EDE8DF", border: showEmojiPicker ? "2px solid var(--accentDark)" : "2px solid transparent", borderRadius:16, fontSize:26, cursor:"pointer", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", transition:"all .15s", boxShadow: showEmojiPicker ? "0 2px 12px rgba(var(--accent-rgb,34,197,94),0.28)" : "none" }}>
            {customEmoji}
          </button>
          <input value={customName} onChange={e => setCustomName(e.target.value)} onKeyDown={e => e.key==="Enter" && addCustom(e)}
      ...
      <div style={{ padding:"10px 16px 6px" }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Buscar artículo..."
          style={{ width:"100%", background: theme.isDark ? "rgba(30,41,59,0.7)" : "rgba(255,255,255,0.80)", border:`1.5px solid ${theme.isDark ? "rgba(99,102,241,0.20)" : theme.border}`, borderRadius:"var(--radius-md,16px)", padding:"11px 16px", color:"var(--textPrimary)", fontSize:14, fontWeight:500, outline:"none", boxSizing:"border-box", transition:"border-color .15s, box-shadow .15s" }}
...
export default function SuperLista() {
  ...
  // ── Android back-gesture / browser back button → return to main ──────────
  // Push ONE dummy entry when leaving home so the OS back gesture has something
  // to pop. Guard with state check so rapid re-renders don't stack up entries.
  useEffect(() => {
    const isHome = view === "lists" && !showProfile;
    if (!isHome && !window.history.state?.sl) {
        ...
        <div style={{
          ...
          <div style={{ position:"relative", flex:1 }}>
            ...
            <input
              ...
              style={{ width:"100%", background: theme.inputBg, border:`2px solid ${theme.inputBorder||theme.border||"#C8DC90"}`, borderRadius:22, padding:"9px 16px 9px 36px", color:"var(--textPrimary)", fontSize:14, outline:"none", boxSizing:"border-box" }}
            />
          </div>
        </div>
      )}
