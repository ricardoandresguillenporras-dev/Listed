...
const SwipeItem = memo(function SwipeItem({ item, onToggle, onQtyMinus, onQtyPlus, onAddToCart, onCartMinus, onDelete, onContextMenu, editingPriceId, tempPrice, setTempPrice, setEditingPriceId, savePrice, sym }) {
  ...
  // Guards handleCheck/handleUncheck/handleQuickCart against firing twice from a
  // fast double-click or a double-tap registering as two separate click events
  // before the 160ms exit animation completes and the row re-renders into its
  // new stage. Without this, two stage-advances land in the same tick and an
  // inventory item can skip straight to "cart" instead of stopping at "shopping".
  ...
  const handleUncheck = useCallback((e) => {
    ...
    Sounds.uncheckItem();
    setExiting(true);
    setTimeout(() => { setExiting(false); onToggle(item.id); actionLockRef.current = false; }, 160);
  }, [item.id, onToggle]);
  ...
  const handleQuickCart = useCallback((e) => {
    ...
    setExiting(true);
    setTimeout(() => { setExiting(false); onAddToCart(item.id); actionLockRef.current = false; }, 160);
  }, [item.id, onAddToCart]);
