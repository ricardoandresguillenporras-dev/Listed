...
function BottomNav({ active, onHome, onSearch, onProfile }) {
  const btn = (Icon, onClick, isActive) => (
    <button onClick={onClick} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"none", border:"none", cursor:"pointer", padding:"6px 0" }}>
      <div style={{ width:52, height:52, borderRadius:16, background:isActive?"#E8F5E8":"transparent", display:"flex", alignItems:"center", justifyContent:"center", transition:"background .15s" }}>
        <Icon size={22} color={isActive?"#fff":"#555"} />
      </div>
    </button>
  );
  return (
    <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:430, maxWidth:"100%", background:"rgba(255,255,255,0.68)", backdropFilter:"blur(20px) saturate(180%)", WebkitBackdropFilter:"blur(20px) saturate(180%)", borderTop:"1px solid rgba(255,255,255,0.65)", boxShadow:"0 -1px 12px rgba(80,60,20,0.06)", display:"flex", alignItems:"center", paddingBottom:4, zIndex:20 }}>
...
function ListsView({ lists, onOpenList, onDeleteList, onCreateList, sym, history, budget }) {
  ...
  const glassCard = {
    ...
    boxShadow: "0 4px 24px rgba(80,60,20,0.09), inset 0 1px 0 rgba(255,255,255,0.8)",
  };
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <span style={{ flex:1, fontWeight:800, fontSize:22, letterSpacing:"-0.03em", color:"#2C2318" }}>
          🛒 SuperLista
        </span>
      </div>

      <div style={{ ...S.body, padding:"14px 0" }}>
        {lists.map((list, idx) => {
          ...
          return (
            <div key={list.id} onClick={() => onOpenList(list.id)}
              style={{
                ...
                padding: "18px 20px",
                cursor: "pointer",
                animation: `itemIn .28s ease ${idx * 0.06}s both`,
                transition: "transform .14s ease, box-shadow .14s ease",
              }}
              onMouseDown={e  => { e.currentTarget.style.transform="scale(.985)"; e.currentTarget.style.boxShadow="0 2px 10px rgba(80,60,20,0.08), inset 0 1px 0 rgba(255,255,255,0.8)"; }}
              onMouseUp={e    => { e.currentTarget.style.transform="scale(1)";    e.currentTarget.style.boxShadow="0 4px 24px rgba(80,60,20,0.09), inset 0 1px 0 rgba(255,255,255,0.8)"; }}
              onTouchStart={e => { e.currentTarget.style.transform="scale(.985)"; }}
              onTouchEnd={e   => { e.currentTarget.style.transform="scale(1)"; }}
            >
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom: total > 0 ? 12 : 6 }}>
                <div>
                  <div style={{ fontSize:17, fontWeight:800, color:"#2C2318", letterSpacing:"-0.01em" }}>{list.name}</div>
                  {total > 0
                    ? <div style={{ fontSize:12, color:"#8A7F72", marginTop:2 }}>{total} artículo{total!==1?"s":""} · {done} en bolsa</div>
                    : <div style={{ fontSize:12, color:"#B0A898", marginTop:2 }}>Toca para añadir</div>
                  }
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {cost > 0 && (
                    <span style={{ fontSize:13, fontWeight:800, color:"#16A34A", background:"rgba(34,197,94,0.12)", borderRadius:10, padding:"3px 9px" }}>
                      {sym}{Math.round(cost).toLocaleString()}
                    </span>
                  )}
                  <button onClick={(e) => { e.stopPropagation(); onDeleteList(list.id); }}
                    style={{ background:"rgba(0,0,0,0.05)", border:"none", color:"#9E9285", fontSize:13, cursor:"pointer", width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                    ✕
                  </button>
                </div>
              </div>
              ...
              {total > 0 && (
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <span style={{ fontSize:11, color:"#9E9285", fontWeight:600, flexShrink:0 }}>{done}/{total}</span>
                </div>
              )}
            </div>
          );
        ...
        {showNew ? (
          <div style={{ ...glassCard, margin:"0 16px 12px", padding:"18px 20px", animation:"itemIn .18s ease both" }}>
            <input ref={inputRef} value={newName} onChange={(e) => setNewName(e.target.value)}
              onKeyDown={(e) => e.key==="Enter" && handleCreate()} placeholder="Nombre de la lista..."
              style={{ width:"100%", background:"rgba(255,255,255,0.6)", border:"1px solid rgba(255,255,255,0.8)", borderRadius:12, padding:"11px 14px", color:"#2C2318", fontSize:15, outline:"none", boxSizing:"border-box", backdropFilter:"blur(8px)" }} />
            <div style={{ display:"flex", gap:10, marginTop:12 }}>
              <button onClick={() => { setShowNew(false); setNewName(""); }}
                style={{ flex:1, background:"rgba(0,0,0,0.06)", border:"1px solid rgba(0,0,0,0.08)", borderRadius:12, padding:"10px 0", color:"#7A6E5F", fontSize:14, cursor:"pointer", fontWeight:600 }}>
                Cancelar
              </button>
              <button onClick={handleCreate}
                style={{ flex:1, background:"linear-gradient(135deg,var(--accent),var(--accentDark))", border:"none", borderRadius:12, padding:"10px 0", color:"#FFFFFF", fontSize:14, fontWeight:800, cursor:"pointer", boxShadow:"0 2px 8px rgba(22,163,74,0.28)" }}>
                Crear ✓
              </button>
            </div>
          </div>
          ...
          <button onClick={() => setShowNew(true)}
            style={{ ...glassCard, margin:"0 16px", width:"calc(100% - 32px)", padding:"15px 0", color:"#6B9E72", fontSize:15, fontWeight:700, cursor:"pointer", border:"1.5px dashed rgba(34,197,94,0.35)", background:"rgba(255,255,255,0.35)", display:"block", textAlign:"center" }}>
            + Nueva lista
          </button>
        )}
      </div>
...
function ListView({ list, onBack, onUpdateItem, onDeleteItem, onGoAdd, sym, budget, onOpenProfile, onSaveBudget, onCloseSession }) {
  ...
  return (
    <>
      <div style={S.header}>
        <button onClick={onBack} style={{ background:"none", border:"none", color:"#16A34A", fontSize:24, cursor:"pointer", padding:"0 6px" }}>←</button>
  ...
  return (
    <>
      <div style={S.header}>
        ...
        <button onClick={onBack} style={{ background:"none", border:"none", color:"#16A34A", fontSize:15, fontWeight:800, cursor:"pointer", padding:"4px 10px", borderRadius:10 }}>Listo ✓</button>
      </div>

      <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px", borderBottom:"1px solid #E4E8E3" }}>
        <input value={customEmoji} onChange={(e) => setCustomEmoji(e.target.value)} maxLength={2}
          style={{ width:40, height:40, background:"#EDE8DF", border:"none", borderRadius:10, fontSize:22, textAlign:"center", color:"#2C2318", outline:"none" }} />
        <input value={customName} onChange={(e) => setCustomName(e.target.value)} onKeyDown={(e) => e.key==="Enter" && addCustom()}
          ...
          style={{ flex:1, background:"#F5F0E8", border:"1px solid #D8DDD6", borderRadius:10, padding:"11px 12px", color:"#2C2318", fontSize:15, outline:"none" }} />
        <button onClick={addCustom} style={{ background:"#22C55E", border:"none", borderRadius:10, width:40, height:40, fontSize:22, color:"#111", fontWeight:800, cursor:"pointer", flexShrink:0 }}>+</button>
      </div>

      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="🔍 Buscar artículo..."
        style={{ margin:"8px 16px", width:"calc(100% - 32px)", background:"#F5F0E8", border:"1px solid #D8DDD6", borderRadius:10, padding:"11px 12px", color:"#2C2318", fontSize:15, outline:"none", boxSizing:"border-box" }} />

      <div style={{ display:"flex", overflowX:"auto", gap:6, padding:"8px 16px", scrollbarWidth:"none" }}>
        {["Todos",...CATEGORIES].map(cat => (
          <button key={cat} onClick={() => setCategory(cat)}
            style={{ borderRadius:20, border:"none", padding:"5px 14px", fontSize:12, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap", flexShrink:0, background:category===cat?"#22C55E":"#EDE8DC", color:category===cat?"#FFFFFF":"#7A6E5F" }}>
            {cat}
          </button>
        ))}
      </div>
      ...
      <div style={{ overflowY:"auto", flex:1 }}>
        {filtered.length===0 ? (
          <div style={{ fontSize:13, color:"#9E9285", padding:20, textAlign:"center" }}>No hay más artículos aquí</div>
        ) : (
          filtered.map(p => {
            const crp = CR_PRICES[p.name];
            return (
              <button key={p.name} onClick={() => addPreset(p)}
                style={{ width:"100%", display:"flex", alignItems:"center", gap:12, padding:"12px 16px", background:"#FFFCF7", border:"none", borderBottom:"1px solid #F4F6F3", cursor:"pointer", color:"#2C2318", textAlign:"left" }}>
                ...
                {crp && <span style={{ fontSize:12, color:"#16A34A", fontWeight:700, marginRight:6, flexShrink:0 }}>{sym}{crp.toLocaleString()}</span>}
                <span style={{ background:"#22C55E", color:"#111", width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:800, flexShrink:0 }}>+</span>
              </button>
            );
          })
        )}
