...
const SwipeItem = memo(function SwipeItem({ item, onToggle, onQtyMinus, onQtyPlus, onDelete, onContextMenu, editingPriceId, tempPrice, setTempPrice, setEditingPriceId, savePrice, sym }) {
  ...
  if (item.checked) {
    return (
      <div ref={wrapRef} style={{ position:"relative", overflow:"hidden", animation: exiting ? "slCheckExit .16s ease forwards" : "slItemBounceIn .38s cubic-bezier(.34,1.56,.64,1) both", contain:"layout style paint", willChange:"transform" }}>
        <div className="sl-bg-left" style={{ position:"absolute", inset:0, background:"rgba(239,68,68,0.12)", opacity:0, display:"flex", alignItems:"center", justifyContent:"flex-end", paddingRight:22, fontSize:14, fontWeight:700, color:"#EF4444", gap:8, transition:"opacity .12s ease" }}>🗑 Eliminar</div>
        <div ref={rowRef}
          style={{ display:"flex", alignItems:"center", padding:"10px 14px", gap:10, background:"rgba(var(--accent-rgb,91,173,114),0.07)", position:"relative", touchAction:"pan-y", userSelect:"none", borderBottom:"1px solid rgba(0,0,0,0.04)", cursor:"pointer", willChange:"transform" }}
  ...
  // ── Unchecked item ─────────────────────────────────────────────────────────
  return (
    <div ref={wrapRef} className="item-stagger" style={{ position:"relative", overflow:"hidden", borderBottom:"1px solid rgba(0,0,0,0.04)", animation: exiting ? "slCheckExit .16s ease forwards" : "slItemSpring .30s var(--ease-spring) both", contain:"layout style paint", willChange:"transform" }}>
      <div className="sl-bg-left"  style={{ position:"absolute", inset:0, background:"rgba(239,68,68,0.12)", opacity:0, display:"flex", alignItems:"center", justifyContent:"flex-end",  paddingRight:22, fontSize:14, fontWeight:700, color:"#EF4444", gap:8, transition:"opacity .12s ease" }}>🗑 Eliminar</div>
      <div className="sl-bg-right" style={{ position:"absolute", inset:0, background:"rgba(var(--accent-rgb),0.13)", opacity:0, display:"flex", alignItems:"center", justifyContent:"flex-start", paddingLeft:22, fontSize:14, fontWeight:700, color:"var(--accentDark)", gap:8, transition:"opacity .12s ease" }}>✓ Seleccionar</div>
      <div ref={rowRef}
        style={{ display:"flex", alignItems:"center", padding:"11px 14px", gap:10, background:"rgba(255,255,255,0.82)", position:"relative", touchAction:"pan-y", userSelect:"none", transition:"background .15s", cursor:"pointer", willChange:"transform" }}
        onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd}
        onMouseDown={onStart} onMouseMove={onMove} onMouseUp={onEnd} onMouseLeave={onEnd}
...
function AddItemsView({ list, onBack, onAddItem, sym, theme = {} }) {
  ...
  return (
    <>
      ...
      {/* ── Custom item row ── */}
      <div style={{ flexShrink:0, padding:"14px 16px 12px", borderBottom:"1px solid #EBE8E2", background:"rgba(255,255,255,0.55)", backdropFilter:"blur(12px)" }}>
