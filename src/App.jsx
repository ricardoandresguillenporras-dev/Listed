...
const makeStyles = (theme) => ({
  ...
  header: {
    ...
    background: theme.isDark ? "#0F172A" : "#F7F1E8",
  ...
  bottomBar: {
    ...
    background: theme.isDark ? "#0F172A" : "#F5EFE5",
...
function BottomNav({ onNewList, onStats, onProfile, active, theme = {} }) {
  ...
  const navBg = theme.navBg || (isDark ? "#151A35" : "#FCFAF6");
  const borderColor = isDark ? `rgba(${theme.accentRgb||"98,121,200"},0.30)` : "#E8DFCD";
  return (
    <div style={{
      position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)",
    ...
    <div style={{ position:"fixed", inset:0, background:"#241D14", zIndex:70, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
      ...
      <div className="wc-sheet" style={{ width:"100%", maxWidth:430, animation:"slideUp .28s cubic-bezier(.34,1.2,.64,1)", maxHeight:"90vh", overflow:"hidden", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", padding:"16px 20px 0", justifyContent:"space-between" }}>
          ...
          <button onClick={onClose} style={{ background:"color-mix(in srgb, var(--accent) 10%, white)", border:"none", color:"var(--textMuted)", width:32, height:32, borderRadius:"50%", fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"background .12s" }}
            onMouseEnter={e=>e.currentTarget.style.background="color-mix(in srgb, var(--accent) 18%, white)"}
            onMouseLeave={e=>e.currentTarget.style.background="color-mix(in srgb, var(--accent) 10%, white)"}>✕</button>
        </div>
          ...
          {tab==="budget" && <>
            ...
            <div style={{ background:"#FBF9F5", borderRadius:"var(--radius-md,16px)", padding:"16px 18px", marginBottom:16, display:"flex", alignItems:"center", gap:10, border:"1.5px solid var(--border)", transition:"border-color .2s, box-shadow .2s" }}
              ...
              {budget && <button onClick={() => setBudget("")}
                style={{ background:"color-mix(in srgb, var(--accent) 10%, white)", border:"none", color:"var(--textMuted)", width:28, height:28, borderRadius:"50%", fontSize:13, cursor:"pointer", flexShrink:0 }}>✕</button>}
            </div>
        ...
        <div style={{ padding:"16px 20px 28px", display:"flex", gap:10 }}>
          <button onClick={onClose} style={{ flex:1, background:"color-mix(in srgb, var(--accent) 8%, white)", border:"1px solid var(--border)", borderRadius:"var(--radius-md,16px)", padding:"13px 12px", color:"var(--textMuted)", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"inherit", transition:"background .12s ease" }}
    ...
    <div style={{ position:"fixed", inset:0, background:"#241D14", zIndex:50, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
      ...
      <div className="wc-context-menu" style={{ width:"100%", maxWidth:430, paddingBottom:20, overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:2, padding:"8px 12px" }}>
          ...
          <div style={{ height:1, background:"color-mix(in srgb, var(--accent) 10%, white)", margin:"4px 8px" }} />
...
function SwipeItem({ item, onToggle, onQtyMinus, onQtyPlus, onDelete, onContextMenu, editingPriceId, tempPrice, setTempPrice, setEditingPriceId, savePrice, sym }) {
  ...
  if (item.checked) {
    return (
      <div ref={wrapRef} style={{ position:"relative", overflow:"hidden", animation:"slItemBounceIn .38s cubic-bezier(.34,1.56,.64,1) both" }}>
        <div className="sl-bg-left" style={{ position:"absolute", inset:0, background:"#FBDADA", display:"none", alignItems:"center", justifyContent:"flex-end", paddingRight:22, fontSize:14, fontWeight:700, color:"#EF4444", gap:8 }}>🗑 Eliminar</div>
        <div ref={rowRef}
          style={{ display:"flex", alignItems:"center", padding:"10px 14px", gap:10, background:"color-mix(in srgb, var(--accent) 9%, var(--wc-paper))", position:"relative", touchAction:"pan-y", userSelect:"none", borderBottom:"1px solid var(--border)", cursor:"pointer" }}
  ...
  return (
    <div ref={wrapRef} className="item-stagger" style={{ position:"relative", overflow:"hidden", borderBottom:"1px solid #EFEAE0", animation:"slItemSpring .30s var(--ease-spring) both" }}>
      <div className="sl-bg-left"  style={{ position:"absolute", inset:0, background:"#FBDADA", display:"none", alignItems:"center", justifyContent:"flex-end",  paddingRight:22, fontSize:14, fontWeight:700, color:"#EF4444", gap:8, transition:"opacity .15s" }}>🗑 Eliminar</div>
      <div className="sl-bg-right" style={{ position:"absolute", inset:0, background:"color-mix(in srgb, var(--accent) 13%, white)", display:"none", alignItems:"center", justifyContent:"flex-start", paddingLeft:22, fontSize:14, fontWeight:700, color:"var(--accentDark)", gap:8, transition:"opacity .15s" }}>✓ Seleccionar</div>
      <div ref={rowRef}
        style={{ display:"flex", alignItems:"center", padding:"11px 14px", gap:10, background:"#FEFCF9", position:"relative", touchAction:"pan-y", userSelect:"none", transition:"background .15s", cursor:"pointer" }}
        onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd}
        onMouseDown={onStart} onMouseMove={onMove} onMouseUp={onEnd} onMouseLeave={onEnd}
...
function AddItemsView({ list, onBack, onAddItem, sym, theme = {} }) {
  ...
  return (
    <>
      ...
      {/* ── Custom item row ── */}
      <div style={{ flexShrink:0, padding:"14px 16px 12px", borderBottom:"1px solid #EBE8E2", background:"#FCFAF6" }}>
      ...
      <div style={{ padding: "16px 16px 0", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {budgetCap > 0 && (
          <div style={cardStyle}>
            ...
            <div style={{ height: 6, background: "color-mix(in srgb, var(--accent) 12%, white)", borderRadius: 3, overflow: "hidden" }}>
              <div style={{
              ...
              {[...sessions].reverse().slice(0, 12).map((s, i, arr) => (
                <div key={s.date + i} style={{
                  ...
                  <div style={{
                    ...
                    background: "color-mix(in srgb, var(--accent) 10%, white)", borderRadius: 8,
                    padding: "2px 8px",
                  }}>
                    {fmtAmt(s.total, sym)}
                  </div>
...
/* Dynamic injected vars (overwritten at runtime) ───────────────── */
        :root {
          ...
          /* bottomBar: 10px*2 vertical padding + 44px input row height
