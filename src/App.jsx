...
const makeStyles = (theme) => ({
  ...
  body: { flex: 1, overflowY: "auto", padding: "8px 0 120px" },
  bottomBar: {
...
const SwipeItem = memo(function SwipeItem({ item, onToggle, onQtyMinus, onQtyPlus, onDelete, onContextMenu, editingPriceId, tempPrice, setTempPrice, setEditingPriceId, savePrice, sym }) {
  ...
  const onMove = useCallback((e) => {
    ...
    if (!s.hasMoved && Math.abs(y) > Math.abs(x) + 3) { s.dragging=false; cancelHold(); return; }
    if (Math.abs(x) > 5) {
      ...
      <div style={Sth.body}>
        ...
        {showUnchecked && unchecked.map(item => (
          <SwipeItem key={item.id} item={item}
            onToggle={(id, forceChecked) => onUpdateItem(id, it => ({ ...it, checked: forceChecked !== undefined ? forceChecked : !it.checked }))}
...
export default function SuperLista() {
  ...
  // Push a dummy history entry whenever the user is NOT on the home screen,
  // so the system back gesture pops it instead of closing the app.
  useEffect(() => {
    const isHome = view === "lists" && !showProfile;
    if (!isHome) {
      // Push a state so there is something to pop back to
      window.history.pushState({ sl: true }, "");
    }
    const handlePop = () => {
      // Any back gesture lands here — always go home
      Sounds.navBack();
      setView("lists");
...
.wc-search-wrap .icon { position:absolute; left:13px; top:50%; transform:translateY(-50%); pointer-events:none; color:var(--textMuted); opacity:0.6; }
      ...
      <div style={Sd.app}>
        {view==="lists" && (
          <div key="lists" className="view-enter-back" onAnimationEnd={clearViewAnim}
            ...
            onTouchEnd={(e) => {
              ...
              if (dy > 80) return;
              if (dx < -60) { Sounds.navBack(); setNavActive("stats"); setView("stats"); }
            }}>
        ...
        {view==="stats" && (
          <div key="stats" className="view-enter-fwd" onAnimationEnd={clearViewAnim} style={{ paddingBottom:80 }}
            ...
            onTouchEnd={(e) => {
              ...
              if (dx > 60 && dy < 80) { Sounds.navBack(); setNavActive("home"); setView("lists"); }
            }}>
            {/* Stats page header */}
            <div style={{
              display:"flex", alignItems:"center", gap:12,
              ...
              <button onClick={() => { Sounds.navBack(); setNavActive("home"); setView("lists"); }}
        ...
        {view==="list" && activeList && (
          <div key="list" className="view-enter-fwd" onAnimationEnd={clearViewAnim}>
          <ListView list={activeList} sym={sym} budget={profile.budget} theme={theme}
            ...
            onBack={() => { setView("lists"); setActiveListId(null); setListSearchQuery(""); }}
        ...
        {view==="addItems" && activeList && (
          <div key="addItems" className="view-enter-fwd" onAnimationEnd={clearViewAnim} style={{ display:"flex", flexDirection:"column", height:"100vh" }}>
      ...
      {(view==="list" || view==="addItems") && view==="list" && activeList && (
        <div style={{
          position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)",
          width:430, maxWidth:"100%",
          background: theme.isDark ? theme.surface : theme.navBg,
